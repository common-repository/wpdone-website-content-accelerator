...
class wp_wpDone_website_content_accelerator {

	const OPTION_GENERAL = 'wwwce_settings';
	...
	public function initialize() {
		if( !class_exists( 'Voce_Settings_API' ) )
			return _doing_it_wrong( __CLASS__, 'The Voce Settings API plugin must be active for the wpDone website content accelerator plugin to work', NULL );
	...
	public function settings_warning() {
		echo "<div class='update-nag'>The wpDone website content accelerator plugin is missing some required settings.</div>";
	}
	...
	public function add_options_page() {
		Voce_Settings_API::GetInstance()->add_page('wpDone website content accelerator', 'wpDone website content accelerator', self::OPTION_GENERAL, 'manage_options', '', 'options-general.php' )
			->add_group( 'Settings', 'wwwce_settings' )
				->add_setting( 'wpDone key  (required, we emailed it to you)', 'root_url', array( 'description' => 'You need to sign up for FREE here, and we\'ll email you a key : https://whmcs.wpdone.com.au/cart.php?gid=3' ) )->group
...
$wwwca_version = get_option('wwwca_version', 1);
		...
		return 'https://wpdonecdn.b-cdn.net/'.$wwwca_version."/".$this->cdn_root_url  . $path;
	}
	...
	public function srcset_rewrite( $match ) {
		...
		foreach( $srcset_images_paths as $key => $original_path ) {
			...
			$wwwca_version = get_option('wwwca_version', 1);
			$cdn_path = 'https://wpdonecdn.b-cdn.net/'.$wwwca_version.'/'.$this->cdn_root_url . $path;
...
add_action('init', array(new wp_wpDone_website_content_accelerator(), 'initialize'));
